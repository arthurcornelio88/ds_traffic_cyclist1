...
""")
    ...
    if modele == "Random Forest":
        st.write("Random Forest est un modèle d'ensemble qui combine plusieurs arbres de décision. Il est robuste et peut gérer des données manquantes et non linéaires. Il est adapté aux données catégorielles et numériques.")
    elif modele == "Réseau de Neurones":
        st.write("Réseau de neurones est un modèle de type réseau de neurones artificiels. Il est capable de gérer des données manquantes et non linéaires. Il est adapté aux données catégorielles et numériques.")
    else :
